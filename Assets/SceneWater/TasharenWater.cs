...
public class TasharenWater : MonoBehaviour
{
	...
	private bool isSupportsRenderDepthTexture=false;
	void Start(){
//		isSupportsRenderDepthTexture=SystemInfo.SupportsRenderTextureFormat (RenderTextureFormat.Depth);
//		if(!isSupportsRenderDepthTexture)Lod=100;
//
//		Light[] lightgos =GameObject.FindObjectsOfType<Light>();
//		foreach(Light l in lightgos){
...
//		}
	...
	/// </summary>
	private static int Lod=300;
	private int perFrames=30;
...
//        }
//		if(!isSupportsRenderDepthTexture)return;
//		if(Fps.Frames==0)return;
//		TotalFrames=TotalFrames+Fps.Frames;
//			countTimes++;
//		if(countTimes>=8){
//			perFrames=TotalFrames/countTimes;
//			countTimes=0;
//			TotalFrames=0;
//			if(perFrames<15 && Lod>100)Lod=Lod-100;
//			else if(perFrames>= 20 && Lod<300)Lod=Lod+100;
//
////			if(dlight!=null && Lod==100){
////				dlight.enabled=false;
////			}else if(dlight!=null && dlight.enabled==false){
////				dlight.enabled=true;
////			}
//		}
	}
	...
	//	if(Camera.current!=null)Camera.current.depthTextureMode=DepthTextureMode.Depth;
