...
Shader "Tasharen/Water"
{
	Properties
	{
		...
		_TilingOffsetX1 ("TilingOffsetX1", float) = 0.5
		_TilingOffsetX2 ("TilingOffsetX2", float) = 0.5
		_TilingOffsetY ("TilingOffsetY1", float) = 0.5
        _TilingOffsetY2 ("TilingOffsetY2", float) = 0.5
	...
	float _Shininess;
	float _Tiling;
	float _TilingOffsetX1;
	float _TilingOffsetY1;
	float _TilingOffsetX2;
	float _TilingOffsetY2;
	...
	SubShader
	{
		...
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			float offsetX1 = _Time.x * _TilingOffsetX1;
			float offsetY1= _Time.x * _TilingOffsetY1;
			float offsetX2 = _Time.x * _TilingOffsetX2;
			float offsetY2= _Time.x * _TilingOffsetY2;
			//float offsety = _Time.x * 0.5;
			half2 tiling = IN.worldPos.xz * _Tiling;
			half4 nmap = (tex2D(_WaterTex, half2(tiling.y+offsetY1, tiling.x+offsetX1)) + tex2D(_WaterTex, half2(-tiling.y-offsetY2, tiling.x-offsetX2) )) * 0.5;
...
#else
		float depth=tex2D(_CameraDepthTexture, IN.proj0.xy/IN.proj0.w).r;
#endif	
	...
	SubShader
	{
		...
		void surf (Input IN, inout SurfaceOutput o)
		{
			...
			// Calculate the object-space normal (Z-up)
			float offset = -_Time.x * 0.5;//Lindean
			half2 tiling = IN.worldPos.xz * _Tiling;
			half4 nmap = (tex2D(_WaterTex, tiling - offset) + tex2D(_WaterTex, half2(-tiling.y, tiling.x) + offset)) * 0.5;
			o.Normal = nmap.xyz * 2.0 - 1.0;
