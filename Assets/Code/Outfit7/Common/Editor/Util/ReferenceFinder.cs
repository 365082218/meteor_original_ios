...
namespace Outfit7.Util {
    ...
    public class AssetUsageDetector : EditorWindow {
        ...
        [MenuItem("Outfit7/Util/Referecne Finder")]
        ...
        [MenuItem("Outfit7/Util/Close Reference Finder %w")]
        ...
        private static void StartComponentSearch(MenuCommand data, bool searchInAllScenes = false, bool searchInAssetsFolder = false) {
            if (data.context != null) {
                ...
                if (comp) {
                    ...
                    O7Log.ErrorT(Tag, "No component selected");
                }
            } else {
                O7Log.ErrorT(Tag, "No data context");
            }
        }
            ...
            if (assetType == AssetType.Shader) {
                ...
                for (int k = 0; k < shaderPropertyCount; k++) {
                    if (ShaderUtil.GetPropertyType(shader, k) == ShaderUtil.ShaderPropertyType.TexEnv
                        && material.GetTexture(ShaderUtil.GetPropertyName(shader, k)) == target) {
                        return true;
                    }
                }
            }
