...
namespace Starlite.Raven {
    ...
    public static class RavenEaseUtility {
        public const double c_PiOver2 = 1.5707963267948966192313216916398;
        public const double c_TwoPi = 6.283185307179586476925286766559;

        public static ERavenEaseType EaseHotkeys(ERavenEaseType currentEaseType) {
            UnityEngine.Event evnt = UnityEngine.Event.current;
            if (EventType.KeyDown == evnt.type && KeyCode.DownArrow == evnt.keyCode) {
                try {
                    return System.Enum.GetValues(typeof(ERavenEaseType)).Cast<ERavenEaseType>()
                        .SkipWhile(e => e != currentEaseType).Skip(1).First();
                } catch (InvalidOperationException e) {
                    e.GetType();
                    return (ERavenEaseType)0;
                }
            } else if (EventType.KeyDown == evnt.type && KeyCode.UpArrow == evnt.keyCode) {
                try {
                    return System.Enum.GetValues(typeof(ERavenEaseType)).Cast<ERavenEaseType>().Reverse()
                        .SkipWhile(e => e != currentEaseType).Skip(1).First();
                } catch (InvalidOperationException e) {
                    e.GetType();
                    return (ERavenEaseType)System.Enum.GetNames(typeof(ERavenEaseType)).Length - 1;
                }
            } else {
        ...
        public static double Evaluate(ERavenEaseType easeType, double time, double duration, double overshootOrAmplitude, double period) {
            switch (easeType) {
                ...
                case ERavenEaseType.InOutSine:
                    return -0.5 * (Math.Cos(Mathf.PI * time / duration) - 1);
        ...
        public static double BounceEaseIn(double time, double duration, double unusedOvershootOrAmplitude, double unusedPeriod) {
            return 1 - BounceEaseOut(duration - time, duration, -1, -1);
        }

        public static double BounceEaseOut(double time, double duration, double unusedOvershootOrAmplitude, double unusedPeriod) {
        ...
        public static double BounceEaseInOut(double time, double duration, double unusedOvershootOrAmplitude, double unusedPeriod) {
