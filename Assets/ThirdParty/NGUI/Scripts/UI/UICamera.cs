...
public class UICamera : MonoBehaviour
{
    ...
    /// true:unpress涔嬪悗璋冪敤currenttouch.dragged鐨凮nDrop鍑芥暟
    static public bool isCurrentDraggedOnDrop = false;
		...
		if (unpressed)
		{
			...
			if (currentTouch.pressed != null)
			{
				...
				else // The button/touch was released on a different object
				{
                    ...
                    if( !isCurrentDraggedOnDrop)
                    {
                        Notify(currentTouch.current, "OnDrop", currentTouch.dragged);
                    }
					else
                    {
                        if (null != currentTouch.dragged)
                        {
                            currentTouch.dragged.SendMessage("OnDrop", currentTouch.current, SendMessageOptions.DontRequireReceiver);
                        }
                    }
                    
				}
			}
            isCurrentDraggedOnDrop = false;
			currentTouch.dragStarted = false;
