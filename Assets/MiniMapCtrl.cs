...
public class MiniMapCtrl : MonoBehaviour {
	...
	void Update () {
        ...
        {
            if (!Notify.ContainsKey(MeteorManager.Instance.UnitInfos[i].InstanceId))
            {
                GameObject obj = null;
                if (MeteorManager.Instance.LocalPlayer == MeteorManager.Instance.UnitInfos[i])
                    obj = GameObject.Instantiate(PlayerPrefab);
                else if (MeteorManager.Instance.UnitInfos[i].Camp == EUnitCamp.EUC_FRIEND)
                    obj = GameObject.Instantiate(FriendPrefab);
                else if (MeteorManager.Instance.UnitInfos[i].Camp == EUnitCamp.EUC_ENEMY)
                    obj = GameObject.Instantiate(EnemyPrefab);
                else
                    obj = GameObject.Instantiate(FlagPrefab);
                Notify.Add(MeteorManager.Instance.UnitInfos[i].InstanceId, obj);
                ...
                obj.name = MeteorManager.Instance.UnitInfos[i].name;
                obj.SetActive(true);
            }
            else
            {
