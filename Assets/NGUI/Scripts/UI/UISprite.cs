...
public class UISprite : UIWidget
{
	...
	public override Color color {
		get {
			if(isGray!=true&&mColor.Equals(Color.black))mColor = new Color(1/255f,1/255f,1/255f);
			return base.color;
		}
		set {
			isGray=false;
    ...
    public void SetGrayEx()
    {
        isGray = true;
        mColor = Color.gray;
    }

    public void SetNormalEx()
    {
        isGray = false;
        mColor = Color.white;
    }
	...
	public void MakePixelPerfect (float ScaleRatio)
	{
		if (!isValid) return;
		...
		if (t == Type.Sliced)
		{
			...
			Vector3 scale = cachedTransform.localScale;
			scale.x = ScaleRatio * (Mathf.RoundToInt(scale.x * 0.5f) << 1);
			scale.y = ScaleRatio * (Mathf.RoundToInt(scale.y * 0.5f) << 1);
			scale.z = 1f;
			cachedTransform.localScale = scale;
		}
		else if (t == Type.Tiled)
		{
			...
			Vector3 scale = cachedTransform.localScale;
			scale.x = ScaleRatio * (Mathf.RoundToInt(scale.x));
			scale.y = ScaleRatio * (Mathf.RoundToInt(scale.y));
			scale.z = 1f;
			cachedTransform.localScale = scale;
		}
		else
		{
			...
			if (tex != null)
			{
				Rect rect = NGUIMath.ConvertToPixels(outerUV, tex.width, tex.height, true);
				float pixelSize = atlas.pixelSize;
				scale.x = ScaleRatio * (Mathf.RoundToInt(rect.width * pixelSize) * Mathf.Sign(scale.x));
				scale.y = ScaleRatio * (Mathf.RoundToInt(rect.height * pixelSize) * Mathf.Sign(scale.y));
				scale.z = 1f;
				cachedTransform.localScale = scale;
			}
