...
public class SceneMng
{
    public void OnEnterLevel(LevelScriptBase levelScript, string sceneItems)
    {
        ...
        else
        {
            for (int i = 0; i < 16; i++)
            {
                Main.Ins.CombatData.GLevelSpawn[i] = WayMng.Instance.wayPoints[i >= WayMng.Instance.wayPoints.Count ? 0 : i].pos;
            }

            for (int i = 0; i < 8; i++)
            {
                Main.Ins.CombatData.GCampASpawn[i] = WayMng.Instance.wayPoints[i >= WayMng.Instance.wayPoints.Count ? 0 : i].pos;
                Main.Ins.CombatData.GCampBSpawn[i] = WayMng.Instance.wayPoints[i >= WayMng.Instance.wayPoints.Count ? 0 : i].pos;
            }
        }
