	...
	Pass {  
		CGPROGRAM
			...
			v2f vert (appdata_t v)
			{
				...
				//v.vertex.xyz = mul(unity_WorldToObject, v.vertex.xyz + _deformsize * _deformref * float3(sin(_deformrange * 3.14 * ((1 - v.texcoord.y) - 0.5f)) * _SinTime.w, sin(_deformrange * 3.14 * (1 - v.texcoord.y)) * _SinTime.w, sin(_deformrange * 3.14 * (1 - v.texcoord.y)) * _SinTime.w));
