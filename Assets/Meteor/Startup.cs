...
public class Startup : MonoBehaviour {
    ...
    void Update () {
        frameIdx++;
        if (Input.GetKeyUp(KeyCode.Return) && cheat != null)
        {
            if (cheat.IsActive())
            {
                string str = cheat.text;
                try
                {
                    if (!string.IsNullOrEmpty(str))
                        ScriptMng.ins.CallString(str);
                }
                catch
                {

                }
                if (!string.IsNullOrEmpty(cheat.text))
                {
                    if (cheats.Count > 10)
                        cheats.RemoveAt(0);
                    cheats.Add(cheat.text);
                }
                cheat.text = "";
            ...
            else
            {
                ...
                cheat.gameObject.transform.SetAsLastSibling();
            }
        }
        ...
        if (Input.GetKeyUp(KeyCode.UpArrow))
        {
            ...
            {
                if (cheatsel <= 0)
                    cheatsel = cheats.Count - 1;
                else
                    cheatsel -= 1;
                if (cheatsel != -1 && cheats.Count > cheatsel)
                    cheat.text = cheats[cheatsel];
            }
        }
