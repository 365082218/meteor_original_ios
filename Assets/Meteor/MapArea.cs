    ...
    public void OnTriggerEnter(Collider other)
    {
        ...
        switch (type)
        {
            ...
            case MapAreaType.FlagBox:
                //进入安全盒，如果身上带有信物，则过关.
                unit = other.gameObject.transform.root.GetComponent<MeteorUnit>();
                if (unit.Attr.IsPlayer && unit.GetFlag && Global.Instance.GLevelMode == LevelMode.SinglePlayerTask)
                    GameBattleEx.Instance.GameOver(1);
                break;
            case MapAreaType.SafeBox:
                //进入通关区域，过关.
                unit = other.gameObject.transform.root.GetComponent<MeteorUnit>();
                if (unit.Attr.IsPlayer && Global.Instance.GLevelMode == LevelMode.SinglePlayerTask)
                    GameBattleEx.Instance.GameOver(1);
                break;
        }
    }
